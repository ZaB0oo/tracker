...
1. **Catalog**: full enumeration of ranked/approved/loved std maps via `/beatmapsets/search`. The osu!web search caps at ~10,000 results per query, so enumeration is **split by rank year**. Cursors are persisted per slice (resumable). Expect ~30–60 min. **DMCA/delisted maps** (removed from search but still ranked) are not seen by enumeration: `POST /api/sync/verify-year/<year>` diffs the search against the local DB and imports delisted sets via direct lookup + web page; `POST /api/sync/import-set/<id>` imports one specific set.
